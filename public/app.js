...
async function dashboard(){const cs=await api('/api/classes');app.innerHTML=`<header><div><b>Minoo Öğretmen</b><small>v2.4.6</small></div><button class="ghost" onclick="home()">Çıkış</button></header><main><div class="toolbar"><h1>Sınıflarım</h1><button onclick="addClass()">+ Sınıf</button></div><div class="grid">${cs.map(c=>`<article class="card"><h3>${esc(c.name)}</h3><p>${c.student_count} öğrenci</p><button onclick="openClass(${c.id})">Aç</button><div class="row"><button class="ghost" onclick="renameClass(${c.id},'${encodeURIComponent(c.name)}')">Adını değiştir</button><button class="danger ghost" onclick="deleteClass(${c.id},'${encodeURIComponent(c.name)}')">Sil</button></div></article>`).join('')}</div><hr><div class="toolbar"><h2>Oyun Kütüphanesi</h2><button onclick="gameForm()">+ Oyun ekle</button></div><div id="library"></div></main>`;loadLibrary()}
...
/* ===== Minoo v2.4.6 • Satranç Taşlarını Dizelim ===== */
let setupState=null;
...
function minooPieceSvg(type,side){
 ...
 else if(type==='knight')body=`<path d="M21 88 C23 75 28 64 36 54 C42 47 47 41 48 34 L40 25 L44 10 L54 23 L62 14 L65 28 C73 30 80 35 85 43 C89 50 89 57 85 63 C81 69 75 72 69 70 C64 68 60 64 56 60 C51 68 48 78 48 88Z" ${F}/><path d="M48 35 C56 29 67 29 76 35 C81 39 84 44 85 49 C82 54 77 57 71 57 C65 57 61 53 56 50" fill="none" stroke="${edge}" stroke-width="3.2" stroke-linecap="round"/><path d="M38 54 C32 64 29 75 30 82" ${H}/><path d="M49 20 Q44 27 44 36" ${H}/><path d="M26 88 H72 L81 98 H17Z" ${B}/><rect x="10" y="98" width="80" height="9" rx="4" ${B}/><circle cx="70" cy="41" r="3.2" fill="${white?'#292724':'#f7f7f7'}"/><path d="M78 58 Q82 60 84 58" fill="none" stroke="${edge}" stroke-width="2.2" stroke-linecap="round"/>`;
 ...
 else body=`<path d="M50 6V25M40 15H60" stroke="${edge}" stroke-width="5" stroke-linecap="round"/><path d="M50 26C35 26 29 37 31 49C32 56 37 61 41 65C35 70 32 75 33 80H67C68 75 65 70 59 65C63 61 68 56 69 49C71 37 65 26 50 26Z" ${F}/><rect x="28" y="79" width="44" height="10" rx="4" ${B}/><path d="M22 89H78L84 101H16Z" ${F}/><path d="M39 33Q32 43 37 53" ${H}/>`;
...
/* ===== Minoo v2.4.6 • Satranç Taşlarının İsimleri ===== */
...
function hearChessName(i){
 ...
 try{
  ...
  u.lang='tr-TR';u.rate=.78;u.pitch=1.08;u.volume=1;
  u.onend=()=>tile?.classList.remove('name-speaking');
