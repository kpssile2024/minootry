...
async function dashboard(){const cs=await api('/api/classes');app.innerHTML=`<header><div><b>Minoo Öğretmen</b><small>v2.6.7</small></div><button class="ghost" onclick="home()">Çıkış</button></header><main><div class="toolbar"><h1>Sınıflarım</h1><button onclick="addClass()">+ Sınıf</button></div><div class="grid">${cs.map(c=>`<article class="card"><h3>${esc(c.name)}</h3><p>${c.student_count} öğrenci</p><button onclick="openClass(${c.id})">Aç</button><div class="row"><button class="ghost" onclick="renameClass(${c.id},'${encodeURIComponent(c.name)}')">Adını değiştir</button><button class="danger ghost" onclick="deleteClass(${c.id},'${encodeURIComponent(c.name)}')">Sil</button></div></article>`).join('')}</div><hr><div class="toolbar"><h2>Oyun Kütüphanesi</h2><button onclick="gameForm()">+ Oyun ekle</button></div><div id="library"></div></main>`;loadLibrary()}
...
const MINOO_VERSION_265='v2.6.7';
...
async function m265Home(){
  try{
    ...
    return home();
  }catch(e){ try{return home()}catch(_){location.reload()} }
}
m262Home=m265Home;
...
/* İngilizce v2.6.7: önce öğretici, sonra 10 aşamalı test */
const M266_ENGLISH={
...
function m265Say(w){try{speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(w);u.lang='en-US';u.rate=.68;u.pitch=1.04;speechSynthesis.speak(u)}catch(e){}}
function m264EnglishHome(){
...
function m266EnglishTeach(k){window._m266en={k,topic:M266_ENGLISH[k],i:0,selected:null,phase:'teach'};m266EnglishRenderTeach()}
function m266EnglishRenderTeach(){const s=window._m266en,t=s.topic;app.innerHTML=`<header><button class="ghost" onclick="m264EnglishHome()">← İngilizce</button><b>🇬🇧 ${t.title} • Öğretici</b><button class="ghost" onclick="m265Home()">⌂ Ana Sayfa</button></header><main class="m266-en"><h1>Önce kelimeleri öğrenelim 🎧</h1><p>Her resme dokun. Resmin İngilizce adını dinle ve tekrar et.</p><div class="m266-learn-grid">${t.items.map((o,i)=>`<button onclick="m265Say(${JSON.stringify(o[1])})"><span>${o[0]}</span><b>${o[1]}</b><small>🔊 Dokun ve dinle</small></button>`).join('')}</div><button class="m266-start" onclick="window._m266en.phase='quiz';window._m266en.i=0;m266EnglishQuiz()">Alıştırmalara Geç →</button></main>`}
function m266EnglishQuiz(){const s=window._m266en,t=s.topic,target=t.items[s.i],n=t.items.length;let ids=[s.i,(s.i+3)%n,(s.i+7)%n];if(new Set(ids).size<3)ids=[s.i,(s.i+2)%n,(s.i+5)%n];ids=ids.sort((a,b)=>((a*7+s.i*3)%11)-((b*7+s.i*3)%11));app.innerHTML=`<header><button class="ghost" onclick="m266EnglishRenderTeach()">← Öğretici</button><b>🇬🇧 ${t.title}</b><span>${s.i+1}/10</span></header><main class="m266-en"><div class="m266-progress"><i style="width:${(s.i+1)*10}%"></i></div><h1>Listen and choose! 👂</h1><p>Kelimeyi dinle. Resimlere dokunduğunda onların adını da duyabilirsin.</p><button class="m265-listen" onclick="m265Say(${JSON.stringify(target[1])})">🔊 Soruyu Dinle</button><div class="m265-enopts">${ids.map(j=>{const o=t.items[j];return `<button class="${s.selected===j?'selected':''}" onclick="m266EnglishPick(${j})"><span>${o[0]}</span><small>🔊 ${o[1]}</small></button>`}).join('')}</div><button onclick="m266EnglishCheck()">✓ Kontrol Et</button></main>`;setTimeout(()=>m265Say(target[1]),250)}
function m266EnglishPick(j){window._m266en.selected=j;m265Say(window._m266en.topic.items[j][1]);document.querySelectorAll('.m265-enopts button').forEach((b,i)=>b.classList.toggle('selected',i===Array.from(b.parentNode.children).indexOf(b)));m266EnglishQuiz()}
function m266EnglishCheck(){const s=window._m266en,t=s.topic;if(s.selected===null)return notify('Önce bir resme dokunalım 👆');if(s.selected!==s.i){m265Say('Try again');document.querySelectorAll('.m265-enopts button').forEach(b=>{if(b.classList.contains('selected'))b.classList.add('m266-wrong')});return notify('Bir daha deneyelim 🌱')}m265Say('Great job! '+t.items[s.i][1]);if(s.i<9){s.i++;s.selected=null;setTimeout(m266EnglishQuiz,550)}else modal(`<h2>Great job! ⭐</h2><p>${t.title} bölümündeki 10 alıştırmayı tamamladın.</p><div class="modal-actions"><button class="ghost" onclick="closeModal();m264EnglishHome()">İngilizce Bölümü</button><button onclick="closeModal();m265Home()">Ana Sayfa</button></div>`)}
...
/* ===== v2.6.7 yönetici/öğretmen + sayaç müziği + At kesin L ===== */
