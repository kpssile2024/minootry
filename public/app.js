...
async function dashboard(){const cs=await api('/api/classes');app.innerHTML=`<header><div><b>Minoo Öğretmen</b><small>v2.3.0</small></div><button class="ghost" onclick="home()">Çıkış</button></header><main><div class="toolbar"><h1>Sınıflarım</h1><button onclick="addClass()">+ Sınıf</button></div><div class="grid">${cs.map(c=>`<article class="card"><h3>${esc(c.name)}</h3><p>${c.student_count} öğrenci</p><button onclick="openClass(${c.id})">Aç</button><div class="row"><button class="ghost" onclick="renameClass(${c.id},'${encodeURIComponent(c.name)}')">Adını değiştir</button><button class="danger ghost" onclick="deleteClass(${c.id},'${encodeURIComponent(c.name)}')">Sil</button></div></article>`).join('')}</div><hr><div class="toolbar"><h2>Oyun Kütüphanesi</h2><button onclick="gameForm()">+ Oyun ekle</button></div><div id="library"></div></main>`;loadLibrary()}
...
/* ===== Minoo v2.3.0 • Satranç Taşlarını Dizelim ===== */
const SETUP_BACK = ['♜','♞','♝','♛','♚','♝','♞','♜'];
const SETUP_WHITE = ['♖','♘','♗','♕','♔','♗','♘','♖'];
...
function renderSetup(){
 ...
 tray.innerHTML=`<div class="setup-tray ${setupState.phase}">${remaining.map(x=>`<button class="tray-piece ${x.side}" draggable="${setupState.phase==='play'}" ondragstart="setupDrag(event,'${x.id}')" onclick="setupPieceClick('${x.id}')" title="${setupPieceName(x)}">${x.p}<small>${setupPieceName(x)} • ${x.side==="white"?"BEYAZ":"SİYAH"}</small></button>`).join('')}</div>`;
...
function setupPieceClick(id){
 ...
 if(setupState.phase==='learn'){
   ...
   if(!from||!to){x.target=key;setupState.placed.set(id,x);setupState.animating=false;renderSetup();return}
   ...
   const fly=document.createElement('div');fly.className=`setup-flying-piece ${x.side}`;fly.textContent=x.p;fly.style.left=(x1-24)+'px';fly.style.top=(y1-24)+'px';document.body.appendChild(fly);requestAnimationFrame(()=>fly.style.transform=`translate(${x2-x1}px,${y2-y1}px)`);
   setTimeout(()=>{svg.remove();fly.remove();x.target=key;setupState.placed.set(id,x);setupState.animating=false;setupPlaceSound();renderSetup();document.querySelector('#setupMessage').textContent=setupState.placed.size===32?'Harika! Taşların doğru dizilişini gördün. Hazırsan oyuna başlayalım!':`${setupPieceName(x)} okla gösterilen yerine yerleşti. Devam et!`;},650);
   return;
 }
