...
async function dashboard(){const cs=await api('/api/classes');app.innerHTML=`<header><div><b>Minoo Öğretmen</b><small>v2.4.5</small></div><button class="ghost" onclick="home()">Çıkış</button></header><main><div class="toolbar"><h1>Sınıflarım</h1><button onclick="addClass()">+ Sınıf</button></div><div class="grid">${cs.map(c=>`<article class="card"><h3>${esc(c.name)}</h3><p>${c.student_count} öğrenci</p><button onclick="openClass(${c.id})">Aç</button><div class="row"><button class="ghost" onclick="renameClass(${c.id},'${encodeURIComponent(c.name)}')">Adını değiştir</button><button class="danger ghost" onclick="deleteClass(${c.id},'${encodeURIComponent(c.name)}')">Sil</button></div></article>`).join('')}</div><hr><div class="toolbar"><h2>Oyun Kütüphanesi</h2><button onclick="gameForm()">+ Oyun ekle</button></div><div id="library"></div></main>`;loadLibrary()}
...
/* ===== Minoo v2.4.5 • Satranç Taşlarını Dizelim ===== */
let setupState=null;
...
function minooPieceSvg(type,side){
 ...
 else if(type==='knight')body=`<path d="M22 87C24 73 30 61 40 51C46 45 48 39 46 32L38 24L43 10L53 24C60 17 69 15 79 19L75 31C82 37 85 45 83 53C81 61 75 66 68 67C61 68 57 64 52 59C48 66 45 76 45 87Z" ${F}/><path d="M46 35C58 28 70 32 78 40L82 48C77 54 71 57 65 57L54 51" ${F}/><path d="M22 87H72L80 97H16Z" ${B}/><rect x="10" y="97" width="80" height="9" rx="4" ${B}/><path d="M43 17Q36 30 39 43" ${H}/><path d="M34 55Q28 68 29 78" ${H}/><circle cx="69" cy="40" r="3" fill="${white?'#333':'#fff'}"/>`;
...
/* ===== Minoo v2.4.5 • Satranç Taşlarının İsimleri ===== */
